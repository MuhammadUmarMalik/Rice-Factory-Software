...
export default function PurchaseReportPage() {
  ...
  const { data, isLoading, error } = useQuery<PurchaseReport>({
    ...
    queryFn: async () => {
      ...
      const res = await apiRequest("GET", `/api/reports/purchases?${params.toString()}`);
      return res.json();
    },
  });
  ...
  useEffect(() => {
    ...
    window.electronLog?.write(`purchase report error: ${message}`);
  }, [error]);
      ...
      render: (item) => (
        <div className={`flex items-center gap-2 ${isRTL ? "flex-row-reverse" : ""}`}>
          ...
          <span className="text-sm">
            {formatSafeDate(item.purchaseDate)}
          </span>
        </div>
      ),
    },
        ...
        <CardContent className="pt-6">
          {error ? (
            <div className="mb-4 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
              {error instanceof Error ? error.message : "Failed to load purchase report"}
            </div>
          ) : null}
          <DataTable
            columns={columns}
