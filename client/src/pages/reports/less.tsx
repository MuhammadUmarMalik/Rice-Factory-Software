...
const reportTitle = "Less / Watta Kaat (Moisture + Quality) Report";
...
export default function LessReportPage() {
  ...
  const { data, isLoading, error } = useQuery<LessReport>({
    queryKey: ["/api/reports/less", fromDate, toDate, supplierId],
    enabled: isReady,
    queryFn: async () => {
      ...
      const res = await apiRequest("GET", `/api/reports/less?${params.toString()}`);
      return res.json();
    },
  });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">{t("lessReport")}</h1>
          <p className="text-sm text-muted-foreground">
            Summary totals for Less / Watta Kaat (Moisture + Quality) deductions.
          </p>
        </div>
        <PrintActions
          docKey={docKeys.lessReport}
          params={{
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <MetricCard title="Total Less / Watta Kaat (Moisture + Quality)" value={totalKg} loading={isLoading} suffix="kg" highlight />
